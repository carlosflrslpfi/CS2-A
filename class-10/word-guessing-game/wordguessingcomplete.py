...
def valid_guess(letter, guessed_letters):
    if (len(letter) != 1 or letter not in alphabeth):
        print("You must guess a letter from the alphabet {}".format(alphabeth))
        return False
...
Good luck!\n\n
    ...
    n = 0
    while (n < 3):
        n = int(input("How long would you like the word to be? (> 2) "))
